...
int
Serial::SerialImpl::getFd () const
{
    return fd_;
}

void
Serial::SerialImpl::setFd (int fd)
{
    fd_ = fd;
...
size_t
Serial::SerialImpl::write (const char *data, size_t length)
{
    if (is_open_ == false) {
...
void
Serial::SerialImpl::raw ()
{
    if (is_open_ == false) {
        throw PortNotOpenedException ("Serial::raw");
    }
